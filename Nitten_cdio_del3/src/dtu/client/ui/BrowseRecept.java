		...
		clientImpl.service.getReceptView(new AsyncCallback<List<ReceptViewDTO>>() {
			...
			public void onSuccess(List<ReceptViewDTO> result) {
				for (int i=0; i < result.size(); i++) {
					...
					for (int p=0; p < result.get(i).getIngredienser().size(); p++){
						
						tIng.setText(p+1, 0, result.get(i).getIngredienser().get(p).getNavn());
						tIng.setText(p+1, 1, Double.toString(result.get(i).getIngredienser().get(p).getNetto()));
					}
					t.setWidget(i+1, 2, tIng);
				}
			}
